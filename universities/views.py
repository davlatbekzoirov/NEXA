...
from .models import (
    University, Scholarship, Document, TestScore, ApplicationTask,
    DocumentVersion, ShareLink, CalendarToken,
)
from .forms import (
    UniversityForm, ScholarshipForm, DocumentForm, TestScoreForm,
    ApplicationTaskForm, DocumentVersionForm,
)
...
class ScholarshipCreateView(View):
    def get(self, request, uni_pk):
        return self._handle(request, uni_pk)

    def post(self, request, uni_pk):
        return self._handle(request, uni_pk)

    def _handle(self, request, uni_pk):
...
@method_decorator(login_required, name='dispatch')
class DocumentVersionUploadView(View):
    def get(self, request, doc_pk):
        return self._handle(request, doc_pk)

    def post(self, request, doc_pk):
        return self._handle(request, doc_pk)

    def _handle(self, request, doc_pk):
...
@method_decorator(login_required, name='dispatch')
class DocumentVersionDeleteView(View):
    def get(self, request, pk):
        version = get_object_or_404(DocumentVersion, pk=pk, document__user=request.user)
        return redirect('universities:document_detail', pk=version.document_id)
...
@method_decorator(login_required, name='dispatch')
class ShareLinkCreateView(View):
    def post(self, request, doc_pk):
        doc = get_object_or_404(Document, pk=doc_pk, user=request.user)
...
@method_decorator(login_required, name='dispatch')
class ShareLinkRevokeView(View):
    def post(self, request, pk):
...
@method_decorator(login_required, name='dispatch')
class CalendarFeedInfoView(View):
...
@method_decorator(login_required, name='dispatch')
class CalendarTokenRegenerateView(View):
    def post(self, request):
        token = CalendarToken.get_or_create_for(request.user)
