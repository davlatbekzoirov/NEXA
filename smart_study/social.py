...
class SocialHelper:
    """Groups accountability-partner / leaderboard helpers as static methods."""
    ...
    def get_leaderboard(user):
        ...
        people = [user] + SocialHelper.get_friends(user)
        board = [
            {
                'user': p,
                'weekly_hours': SocialHelper.get_weekly_hours(p),
                'streak': SocialHelper.get_streak(p),
            }
            for p in people
        ]
